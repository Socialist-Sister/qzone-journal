...
The renderer may request a collection job through a narrow preload API, but it never receives cookies, passwords, or unrestricted filesystem access. The Electron main process validates every destination path and job option before starting an Electron Utility Process. The login window and collector share the `persist:qzone-journal-account` partition, so raw cookies do not cross the IPC boundary.
